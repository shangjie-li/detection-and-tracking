...
def fit_2d_oriented_bounding_box(xs, ys):
    # 功能：拟合2D带方向包围盒
    # 输入：xs <class 'numpy.ndarray'> (n,) 代表横坐标
    #      ys <class 'numpy.ndarray'> (n,) 代表纵坐标
    ...
    if max_idx < min_idx:
        remain_list = list(range(max_idx, min_idx + 1))
    else:
        remain_list = list(range(max_idx, num)) + list(range(0, min_idx + 1))
    polygon_remained = polygon_sorted[remain_list, :, :]
    ...
    # STEP3
    # 在保留的轮廓中寻找最长线段
    num = polygon_approximated.shape[0]
    pdd = 0
    xm1, ym1, xm2, ym2 = 0, 0, 0, 0
    for j in range(num - 1):
        x1 = polygon_approximated[j, 0, 0]
        y1 = polygon_approximated[j, 0, 1]
        x2 = polygon_approximated[j + 1, 0, 0]
        y2 = polygon_approximated[j + 1, 0, 1]
        dd = (x2 - x1) ** 2 + (y2 - y1) ** 2
        if dd > pdd:
            xm1, ym1, xm2, ym2 = x1, y1, x2, y2
            pdd = dd
    ...
    # 投影至局部坐标系
    xsp, ysp, xp1, yp1, xp2, yp2 = project_point_clouds_on_line(xs, ys, 0, 0, angle)
    l = math.sqrt((xp1 - xp2) ** 2 + (yp1 - yp2) ** 2)
    xsp_second, ysp_second, xp3, yp3, xp4, yp4 = project_point_clouds_on_line(xs, ys, 0, 0, angle_second)
    ...
    return x0, y0, l, w, phi, polygon, polygon_approximated
    
def fit_3d_oriented_bounding_box(xs, ys, zs):
    # 功能：拟合3D带方向包围盒
    # 输入：xs <class 'numpy.ndarray'> (n,) 代表X坐标
    #      ys <class 'numpy.ndarray'> (n,) 代表Y坐标
    #      zs <class 'numpy.ndarray'> (n,) 代表Z坐标
    # 输出：x0 <class 'float'> 代表3D包围盒中心点X坐标
    #      y0 <class 'float'> 代表3D包围盒中心点Y坐标
    #      z0 <class 'float'> 代表3D包围盒中心点Z坐标
    #      l <class 'float'> 代表3D包围盒的长
    #      w <class 'float'> 代表3D包围盒的宽
    #      h <class 'float'> 代表3D包围盒的高
    #      phi <class 'float'> 代表方向角，亦为XY平面内长边与X轴的夹角，范围[0, pi)
    
    x0, y0, l, w, phi, _, _ = fit_2d_oriented_bounding_box(xs, ys)
    ...
    return x0, y0, z0, l, w, h, phi
    
def transform_2D_point_clouds(xs, ys, phi, x0, y0):
    # 功能：对平面点云进行旋转、平移
    # 输入：xs <class 'numpy.ndarray'> (n,) 代表横坐标
    #      ys <class 'numpy.ndarray'> (n,) 代表纵坐标
    #      phi <class 'float'> 为旋转角度，范围[0, 2pi)
...
if __name__ == "__main__":
    ...
    # 导入测试数据
    pts = np.array([xs10, ys10]).T
    print('pts')
    print(pts.shape)
    
    # 拟合包围盒
    x0, y0, l, w, phi, polygon, polygon_approximated = fit_2d_oriented_bounding_box(pts[:, 0], pts[:, 1])
    
    # 计算包围盒顶点坐标
    obb_xs, obb_ys = compute_obb_vertex_coordinates(x0, y0, l, w, phi)
    
    time_cost = time.time() - time_start
    print('time cost of the process:', time_cost)
    ...
    # 在画布中创建1行4列的坐标轴实例，画布大小8inch*3inch
    ...
    for i in range(polygon_approximated.shape[0] - 1):
        x1 = polygon_approximated[i, 0, 0]
        y1 = polygon_approximated[i, 0, 1]
        x2 = polygon_approximated[i + 1, 0, 0]
        y2 = polygon_approximated[i + 1, 0, 1]
